...
import {doOnNextEventLoop, Io, RecordKey} from "@sagittal/general"
...
const getUnicode = (userInput: Io, clef: Clef): Uni => {
    const CLEF_UNICODE_MAP = clef === Clef.BASS ?
...
const doTheThing = (): void => {
    staffDiv.textContent = input.value
        .toLowerCase()
        .replace(/<br>/g, " ")
        .replace(/\n/g, " ")
        .replace(/\t/g, " ")
...
input.value = "st24 tbcf sp16 st24"
input.addEventListener("keydown", (): void => { doOnNextEventLoop(doTheThing) })
input.addEventListener("paste", (): void => { doOnNextEventLoop(doTheThing) })
input.addEventListener("cut", (): void => { doOnNextEventLoop(doTheThing) })

const staffDiv = document.createElement("div")
staffDiv.style.fontFamily = "Bravura Text BB"
staffDiv.style.fontSize = "72px"
